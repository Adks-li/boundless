...
# Patch: 替换 Pinata IPFS 网关为 ipfs.io
    PATCH_FILE="$INSTALL_DIR/scripts/bento-deploy.sh"
    if grep -q "gateway.pinata.cloud" "$PATCH_FILE"; then
        info "Patching IPFS gateway in bento-deploy.sh..."
        sed -i 's#https://gateway.pinata.cloud/ipfs/#https://ipfs.io/ipfs/#g' "$PATCH_FILE"
        success "Patched IPFS gateway successfully"
    fi
}
